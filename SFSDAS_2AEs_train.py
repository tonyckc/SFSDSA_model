...
"""Data Saving Dir"""
TEST_NAME = "MAE_test_re=" + str(re_num) + '_lr=' + str(lr_num)
curdir = os.getcwd()
...
AE = Autoencoder(n_layers=[MODEL_SIZE[0], MODEL_SIZE[1]],
                 ...
                 ae_para=[CORRUPTION_LEVEL, SPARSE_REG], is_first=True, regularzation_rate=REGULARZATION_RATE)
AE_2 = Autoencoder(n_layers=[MODEL_SIZE[1], MODEL_SIZE[2]],
                   transfer_function=tf.nn.selu,
                   optimizer=tf.train.AdamOptimizer(learning_rate=LEARNING_RATE),
                   ae_para=[CORRUPTION_LEVEL, SPARSE_REG], is_first=False, regularzation_rate=REGULARZATION_RATE)
